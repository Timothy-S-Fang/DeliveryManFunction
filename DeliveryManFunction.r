...
myFunction <- function(trafficMatrix, carInfo, packageMatrix) {
  ...
  # Function that finds fastest path to closest package
  # Function that finds fastest path to closest destination
...
getManhattenDistance <- function(p1, p2) {
  return(abs(p1[1] - p2[1]) + abs(p1[2], p2[2]))
}

# Find the goal location (ie. nearest package or next delivery)

findGoal <- function(packageMatrix) {
  packageMatrix
  pickUpList = which(packageMatrix[])
  for (i in 1:length(packageMatrix))
